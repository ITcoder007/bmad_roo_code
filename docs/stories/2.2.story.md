...
## Tasks / Subtasks
- [ ] Task 1: 创建预警规则数据模型 (AC: 1)
  - [ ] Subtask 1.1: 创建 `domain/model/AlertRule.java` 实体类，定义预警规则属性
  - [ ] Subtask 1.2: 创建 `domain/model/AlertRuleType.java` 枚举，定义预警规则类型
  - [ ] Subtask 1.3: 创建 `domain/model/AlertTriggerCondition.java` 实体类，定义触发条件
  - [ ] Subtask 1.4: 定义预警规则的业务验证逻辑

- [ ] Task 2: 实现预警规则引擎 (AC: 2)
  - [ ] Subtask 2.1: 创建 `service/AlertRuleEngine.java` 接口，定义规则引擎契约
  - [ ] Subtask 2.2: 创建 `service/impl/AlertRuleEngineImpl.java` 实现类
  - [ ] Subtask 2.3: 实现 `evaluateRules(Certificate certificate)` 方法，判断是否触发预警
  - [ ] Subtask 2.4: 实现 `getTriggeredRules(Certificate certificate)` 方法，获取被触发的规则
  - [ ] Subtask 2.5: 实现规则优先级处理逻辑

- [ ] Task 3: 创建预警规则配置服务 (AC: 3)
  - [ ] Subtask 3.1: 创建 `service/AlertRuleConfigService.java` 接口
  - [ ] Subtask 3.2: 创建 `service/impl/AlertRuleConfigServiceImpl.java` 实现类
  - [ ] Subtask 3.3: 实现预警规则的加载和缓存机制
  - [ ] Subtask 3.4: 实现基础预警规则的初始化（30天、15天、7天、1天）
  - [ ] Subtask 3.5: 实现规则配置的验证逻辑

- [ ] Task 4: 集成规则引擎到监控服务 (AC: 2)
  - [ ] Subtask 4.1: 修改 `MonitoringServiceImpl.java`，集成预警规则引擎
  - [ ] Subtask 4.2: 在监控流程中调用规则引擎判断是否需要发送预警
  - [ ] Subtask 4.3: 实现基于规则结果的预警触发逻辑
  - [ ] Subtask 4.4: 添加规则评估的日志记录

- [ ] Task 5: 编写单元测试 (AC: 4)
  - [ ] Subtask 5.1: 创建 `test/service/AlertRuleEngineTest.java` 测试类
  - [ ] Subtask 5.2: 编写 `testEvaluateRules_30DaysRule()` 测试方法
  - [ ] Subtask 5.3: 编写 `testEvaluateRules_MultipleDaysRules()` 测试方法
  - [ ] Subtask 5.4: 编写 `testGetTriggeredRules()` 测试方法
  - [ ] Subtask 5.5: 编写规则引擎边界情况的测试用例
  - [ ] Subtask 5.6: 创建 `test/service/AlertRuleConfigServiceTest.java` 测试配置服务

- [ ] Task 6: 集成测试和功能验证 (AC: 5)
  - [ ] Subtask 6.1: 创建集成测试，验证规则引擎与监控服务的集成
  - [ ] Subtask 6.2: 创建测试数据，包含不同到期天数的证书
